...
description: |
   ...
   **Primary Use Cases:**
   ...
   - Writing Kratos middleware and handling Kratos-specific errors
   ...
   **When to Use This Agent:**
   ...
   - Modifying wire.go, wire_gen.go, or provider configurations
   - Creating or updating .proto files in api/ or internal/conf/
   - Implementing GORM repositories, entities, or database queries
   ...
   **DO NOT USE for:** GitHub Actions, CI/CD pipelines, Docker builds, Kubernetes deployments, workflow automation, or .github/workflows files.
...
7. **Test**: Write comprehensive tests
   - Analyze the code and understand its purpose before generating test cases
   - Avoid testing the implementation.. test the business logic
   - Use table-driven tests with testify
   - Mock repositories with testify/mock
   - Test business logic thoroughly
   - Run `make test` or `make coverage`

8. **Test Review**: Review tests
   - See all tests and analyze for overengineering
   - see if there are no redundant tests
   - see if there are any missing tests
   - see if there are any tests that are too specific
   - see if there are any tests that are too broad

9 **Build and Run**: Build and verify
